    ...
    if current_user.is_authenticated:
        ...
        if vaccine_details:
            ...
            if vaccine.doses_req > vaccine_details.dose_count:
                ...
                result['next_dose'] = vaccine_details.dose_date + timedelta(vaccine_details.dose_count*vaccine.dose_time)
                print(result)
    ...
    if vaccinations:
        ...
        for details in vaccinations:
            ...
            if details.dose_count < Vaccine.query.get(details.vaccine_id).doses_req:
                complete_profile.append({"user":details,"Name":f"{user.first_name} {user.last_name}","Vaccine":f"{vaccine.name}"  ,'status':False})
            else:
                complete_profile.append({"user": details, "Name":f"{user.first_name} {user.last_name}","Vaccine":f"{vaccine.name}",'status': True})
        ...
        if Vaccination_Detail.query.filter_by(user_id=user_id).first():
            ...
            if prev_vaccine.id != int(vaccine):
                flash(
                    ...
                    'danger')
                return render_template("vaccinate.html", user=User.query.get(prev_record.user_id), form=form, vaccine = Vaccine.query.all())
            ...
            if prev_record.dose_count == prev_vaccine.doses_req:
                flash(f"Congratulate {User.query.get(prev_record.user_id).first_name} for being fully vaccinated", 'success')
